...
def load_img(image_path, nFrame, scale, num):
    ...
    for i in seq:
       if i != 0:
           index = int(image_path[char_len-8:char_len-4]) + i
           image = image_path[0:char_len-8]+'{0:04d}'.format(index)+'.png'
...
class DataloadFromFolderTest(data.Dataset): # load test dataset
    def __init__(self, image_dir, file_list, scale, nFrame, transform):
        ...
        self.image_filenames = sorted([os.path.join(image_dir, x) for x in os.listdir(image_dir)], key=lambda x: int(x[-8:-4]))
        print(self.image_filenames)
        self.scale = scale
        self.transform = transform # To_tensor
        self.nFrame = nFrame
    def __getitem__(self, index):
        LR = load_img(self.image_filenames[index], self.nFrame, self.scale, len(self.image_filenames)) 
        LR = [np.asarray(T) for T in LR] 
        LR = np.asarray(LR)
        #if self.scale == 4:
        ...
        LR = LR.view(c,t,h,w)
        target = LR[:,3,:,:]
        #LR = GT#Guassian_downsample(GT, self.scale)
